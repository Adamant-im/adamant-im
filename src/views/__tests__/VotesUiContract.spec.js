...
describe('Votes UI style contract', () => {
  it('uses shared settings table shell for delegates screen layout', () => {
    ...
    expect(votesContent).toContain('padding-bottom: var(--a-space-4);')
    expect(votesContent).toContain('--a-delegates-review-button-margin')
    expect(votesContent).toContain('&__review-button')
    expect(votesContent).toContain('&__pagination')
    expect(votesContent).not.toContain('class="w-100"')
    expect(votesContent).not.toContain('margin-left: auto !important;')
  ...
  it('uses shared data table surface and tokenized delegates table cells', () => {
    ...
    expect(headContent).toContain('th.delegates-table-head__td')
    ...
    expect(headContent).not.toContain('border-bottom: none !important;')
    ...
    expect(itemContent).toContain('td.delegates-table-item__td')
    ...
    expect(itemContent).not.toContain('border-bottom: none !important;')
  })
  ...
  it('uses shared compact spacing tokens in delegate details expander', () => {
    ...
    expect(expanderContent).toContain('label: `${className}__label`')
    expect(expanderContent).toContain('&__label')
    expect(expanderContent).toContain('@include mixins.a-text-explanation();')
    expect(expanderContent).toContain('value: `${className}__value`')
    expect(expanderContent).toContain('&__value')
    expect(expanderContent).toContain('min-height: var(--a-delegate-details-expander-item-height);')
    expect(expanderContent).not.toContain('class="a-text-explanation text-right"')
    expect(expanderContent).not.toContain('class="a-text-explanation"')
  ...
  it('uses shared tokens for delegates vote checkbox and loading row spacing', () => {
    ...
    expect(loaderContent).toContain('@include mixins.a-text-regular();')
    ...
    expect(loaderContent).not.toContain('class="a-text-regular"')
  ...
  it('uses shared tokens for votes dialog width and delegates not-found spacing', () => {
    ...
    expect(notFoundContent).toContain('color="var(--a-color-text-muted-light)"')
