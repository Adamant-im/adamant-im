...
describe('Financial UI style contract', () => {
  ...
  it('syncs SendFundsForm currency with store currentWallet', () => {
    ...
    expect(content).toContain('<div v-if="addressReadonly" class="fake-input">')
    expect(content).toContain('readonlyRecipientLabel')
    expect(content).toContain("{{ $t('transfer.crypto') }}")
