  ...
  <div class="chat">
      <md-layout md-align="start" md-gutter="16" class="chat_messages">
          <md-layout v-for="message in messages" :key="message.id" md-flex="100">
              <md-layout v-if="message.direction=='to'" md-flex="80" md-flex-xsmall="70" class=" chat_message " md-flex-offset="5" >{{ message.message }}<div class="dt">{{ dateFormat(message.timestamp) }}</div></md-layout>
              <md-layout v-if="message.direction=='from'" md-flex="80" md-flex-xsmall="70" class=" chat_message md-own md-flex-xsmall-offset-20" md-flex-offset="10" >{{ message.message }} <div class="dt">{{ dateFormat(message.timestamp) }}</div></md-layout>
          </md-layout>
      </md-layout>
