            ...
            <md-table-body v-bind:style="tableStyle">
              <template v-for="(delegate, index) in delegates">
                <md-table-row style="cursor:pointer" :key="delegate.address" v-bind:class="{upvoted: delegate.upvoted, downvoted: delegate.downvoted}" @click.native="toggleDetails(delegate)">
                  ...
                  <md-table-cell>
                    <md-icon>{{ delegate.showDetails ? 'show_less' : 'chevron_right' }}</md-icon>
                  </md-table-cell>
                </md-table-row>
                <md-table-row v-show="delegate.showDetails">
                  <md-table-cell colspan="4" class="delegate-details">
                    <span class="block">
...
<script>
  export default {
    ...
    methods: {
      ...
      testC (delegate) {
        alert('Wooopiiiiie!, ' + delegate.username)
      }
    },
    computed: {
      delegates () {
